...
def dump_schema(output_file_path):
    ...
    rapidapi_auth = {"type": "apiKey", "name": "x-rapidapi-key", "in": "header"}
    ...
    for path, methods in openapi_data.get("paths", {}).items():
        for method, details in methods.items():
            if isinstance(details, dict):  # Ensure we're modifying the correct level
                ...
                details["parameters"].append({"name": "x-rapidapi-key", "in": "header", "required": True, "schema": {"type": "string", "example": "<YOUR_RAPIDAPI_KEY>"}})

                # Hardcode x-rapidapi-host header
                details["parameters"].append({"name": "x-rapidapi-host", "in": "header", "required": True, "schema": {"type": "string", "example": RAPIDAPI_HOST}})
