...
function displayTarotCard(data, container) {
    ...
    const status = data.is_reversed ? '🔄 وارونه' : '⬆️ راست';
    ...
    container.innerHTML = `
        <div style="background: rgba(108,92,231,0.1); border: 1px solid rgba(108,92,231,0.3); border-radius: 16px; padding: 20px; margin-top: 10px;">
            <div style="display: flex; gap: 20px; align-items: flex-start; flex-wrap: wrap;">
                <img src="${imgUrl}" style="width: 140px; height: auto; border-radius: 12px; border: 2px solid rgba(255,255,255,0.15); flex-shrink: 0;" alt="${card.name || ''}" onerror="this.src='/static/tarot/images/placeholder.webp'">
                <div style="flex: 1; min-width: 200px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
                        <h4 style="color: #fdcb6e; margin: 0;">${card.name || 'بدون نام'}</h4>
                        <span style="background: ${data.is_reversed ? '#e17055' : '#00b894'}; padding: 4px 14px; border-radius: 20px; color: #fff; font-size: 0.8rem;">${status}</span>
                    </div>
...
function displayTarotCards(cards, container) {
    ...
    cards.forEach((item, index) => {
        ...
        const status = item.is_reversed ? '🔄 وارونه' : '⬆️ راست';
        const imgUrl = card.image || '/static/tarot/images/placeholder.webp';
        html += `
            <div style="background: rgba(108,92,231,0.08); border: 1px solid rgba(108,92,231,0.2); border-radius: 12px; padding: 15px; margin-top: 10px; display: flex; gap: 15px; align-items: flex-start;">
                <img src="${imgUrl}" style="width: 80px; height: auto; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); flex-shrink: 0;" alt="${card.name || ''}" onerror="this.src='/static/tarot/images/placeholder.webp'">
                <div style="flex: 1;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <h5 style="color: #fdcb6e; margin: 0;">#${index+1}: ${card.name || 'بدون نام'}</h5>
                        <span style="background: ${item.is_reversed ? '#e17055' : '#00b894'}; padding: 2px 12px; border-radius: 20px; color: #fff; font-size: 0.7rem;">${status}</span>
                    </div>
